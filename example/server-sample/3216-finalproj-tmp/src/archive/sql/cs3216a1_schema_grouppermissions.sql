...
CREATE TABLE `grouppermissions` (
  ...
  `eventID` bigint(64) unsigned NOT NULL,
  `permissionLevel` enum('read','readWrite') NOT NULL DEFAULT 'read',
  PRIMARY KEY (`groupID`,`eventID`),
  KEY `eventID_gp_fk_idx` (`eventID`),
  CONSTRAINT `eventID_gp_fk` FOREIGN KEY (`eventID`) REFERENCES `issues_fb` (`IssueID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `groupID_gp_fk` FOREIGN KEY (`groupID`) REFERENCES `groups` (`groupID`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
-- Dump completed on 2014-08-31 22:05:52
